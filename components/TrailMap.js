...
export default function TrailMap({ trails, selectedTrail, onSelectTrail }) {
  ...
  useEffect(() => {
    ...
    // Wait one frame so the container has a concrete pixel size before init
    const initMap = () => {
      ...
      map.current.on('load', () => {
        setMapLoaded(true)
        // Force a resize after load to ensure tiles fill the container correctly
        map.current.resize()
      })

      // Also resize whenever the container element changes size
      const ro = new ResizeObserver(() => {
        if (map.current) map.current.resize()
      })
      ro.observe(mapContainer.current)

      return () => { ro.disconnect(); map.current?.remove() }
    }
  ...
  useEffect(() => {
    ...
    trails.forEach(trail => {
      ...
      const trailPopup = new mapboxgl.Popup({ offset: 20, closeButton: false })
        .setHTML(`
          <div style="font-family:system-ui,sans-serif;padding:4px 0;min-width:180px">
            ...
            <div style="font-size:11px;color:#555;margin-bottom:2px">${(trail.transitMin || 0) + (trail.walkMin || 0)} min total</div>
            <div style="margin-top:6px;padding-top:6px;border-top:1px solid #eee">
              ...
              <div style="font-size:11px;color:#555">${trail.line || trail.operator || ''}</div>
      ...
      // Station marker — only when selected
      if (isSelected && trail.stationCoords) {
        const stEl = document.createElement('div')
        stEl.style.cssText = `
          width: 28px; height: 28px; border-radius: 50%;
          background: #1a56c4; border: 2px solid #0c3d8a;
    ...
    return (
      <div className="w-full h-full rounded-xl bg-gray-100 flex items-center justify-center">
        <div className="text-center text-gray-500 text-sm p-6">
          ...
          <p className="text-xs">Add your Mapbox token to enable the map.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-gray-200 relative" style={{ width: '100%', height: '100%' }}>
